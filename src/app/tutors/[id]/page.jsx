...
export default function TutorDetailsPage() {
  ...
  const isDateValid = today >= sessionDate;
  const hasSlots = totalSlot > 0;
  const canBook = isDateValid && hasSlots;

  const handleBook = () => {
    if (!hasSlots) return toast.error("No available slots left");
    if (!isDateValid)
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 space-y-8">
          ...
          <div className="flex gap-3 flex-wrap text-sm">
            ...
            <span
              className={`px-3 py-1 rounded-full ${
                hasSlots ? "bg-green-100" : "bg-red-100"
              }`}
            >
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="p-5 border rounded-xl space-y-2">
          <h2 className="font-bold">Teaching Mode</h2>
          <p>{tutor.teachingMode}</p>
        </div>

        <div className="p-5 border rounded-xl space-y-2">
          <h2 className="font-bold">Session Start Date</h2>
