          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-10">
            {steps.map((step) => (
              <div key={step.id} className="relative group">
                ...
                {/* CARD */}
                <div className="pt-12 h-full flex flex-col rounded-[32px] bg-linear-to-b from-cyan-50 to-white dark:from-white/5 dark:to-white/3 border border-black/5 dark:border-white/10 p-8 shadow-xl hover:-translate-y-3 transition-all duration-500 relative overflow-hidden">
                  {/* ICON */}
                  <div className="w-20 h-20 rounded-3xl bg-linear-to-br from-cyan-500 to-blue-600 text-white text-3xl flex items-center justify-center shadow-2xl mx-auto shrink-0">
                    {step.icon}
                  </div>

                  {/* TITLE */}
                  <h3 className="mt-8 text-2xl font-black text-center text-black dark:text-white min-h-[64px] flex items-center justify-center">
                    {step.title}
                  </h3>

                  {/* DESCRIPTION */}
                  <p className="mt-4 text-default-600 leading-relaxed text-center flex-1">
                    {step.description}
                  </p>
